...
def get_parameter():
    ...
    parser.add_argument('-chr', required=True, type=int, help='int of chromosome number, X is 23')
    ...
    parser.add_argument('-res', type=int, help='bin resolution. Required if the input is not .byChr file. For example, txt or csv')
    parser.add_argument('-nolog', action='store_true', help='invoke if need heatmap without log. Default is log version')
    parser.add_argument('-name', type=str, default='hm', help='any suffix want to add in the fig name')
    parser.add_argument('-zMax', type=float, help='zMax of heatmap')
    parser.add_argument('-zMin', type=float, help='zMin of heatmap')
...
def region_plot(hm_file, chr, resolution, **kwargs):
    if 'region' in kwargs:
        ...
        end_bin = int(region_end/resolution) + 1
    else:
        start_bin = 0
        if re.search('.byChr', hm_file):
            end_bin = h5dict.h5dict(hm_file)['{} {}'.format(chr, chr)].shape[0]
        else:
            new = np.genfromtxt(hm_file, delimiter=",", filling_values=0, missing_values='NaN')
            end_bin = new.shape[0]
        region_start = 0
    ...
    if 'ratio' not in kwargs:
        if re.search('.byChr', hm_file):
            ...
            print(end_bin)
            try:
                region_hm = new[start_bin:end_bin, start_bin:end_bin]
            except:
                new = np.genfromtxt(hm_file, delimiter=",", filling_values=0, missing_values='NaN')
                region_hm = new[start_bin:end_bin, start_bin:end_bin]
    else:
        if re.search('.byChr', hm_file):
            ...
            # TODO: detect if h5dict format
            region_hm = new[start_bin:end_bin, start_bin:end_bin]
            region_hm1 = new[start_bin:end_bin, start_bin:end_bin]
            region_hm = region_hm/region_hm1
    ...
    if args.nolog:
        if 'ratio' not in kwargs:
            plot_matrix(ax1, region_hm, cmap='afmhot_r')
        else:
            plot_matrix(ax1, region_hm, cmap='bwr')
    else:
        if 'ratio' not in kwargs:
            plot_matrix(ax1, np.log(region_hm), cmap='afmhot_r')
        else:
            plot_matrix(ax1, np.log(region_hm), cmap='bwr')
    ...
    if 'ratio' in kwargs:
        plt.savefig('{}_chr{}_{}_{}_{}_{}_ratio.pdf'.format(sample, chr+1, region_start, region_end, str(resolution/1000)+'k', args.name), dpi=600)
    else:
        plt.savefig('{}_chr{}_{}_{}_{}_{}'.format(sample, chr+1, region_start, region_end, str(resolution/1000)+'k', args.name)+'.pdf', dpi=600)
    ...
    if args.s is None:
        if args.isfile is not None:
            if args.isfile1 is not None:
                tads1 = processInsulationScoreResult(args.isfile1, args.is_threshold, resolution)
            tads = processInsulationScoreResult(args.isfile, args.is_threshold, resolution)
            if args.r is not None:
                region_plot(args.hm, chrom, resolution, region=args.r, tads=tads)
            else:
                region_plot(args.hm, chrom, resolution, tads=tads)
        elif args.r is not None:
            region_plot(args.hm, chrom, resolution, region=args.r)
        else:
            region_plot(args.hm, chrom, resolution)
    else:
        if args.isfile is not None:
            tads = processInsulationScoreResult(args.isfile, args.is_threshold, resolution)
            if args.r is not None:
                region_plot(args.hm, chrom, resolution, region=args.r, tads=tads, ratio=True)
            else:
                region_plot(args.hm, chrom, resolution, tads=tads, ratio=True)
        elif args.r is not None:
            region_plot(args.hm, chrom, resolution, region=args.r, ratio=True)
        else:
            region_plot(args.hm, chrom, resolution, ratio=True)
"""
    ...
    if args.s is None:
        ...
        if args.r is not None:
            ...
            if tads is None and tads1 is not None:
                logging.warning('isfile1 is invoked without -s, exiting')
                exit()
                # region_plot(args.hm, chrom, resolution, tads1=tads1)
            if tads is not None and tads1 is None:
                region_plot(args.hm, chrom, resolution, tads=tads)
            if tads is not None and tads1 is not None:
                logging.warning('isfile1 is invoked without -s, exiting')
                exit()
                # region_plot(args.hm, chrom, resolution, tads=tads, tads1=tads1)
    else:
        if args.isfile is not None:
            # tads = processInsulationScoreResult(args.isfile, args.is_threshold, resolution, gap)
            tads = processInsulationScoreResult_slow(args.isfile, args.is_threshold, resolution, args.hm)
        ...
        if args.r is not None:
            if tads is None and tads1 is None:
                region_plot(args.hm, chrom, resolution, region=args.r, ratio=True)
            if tads is None and tads1 is not None:
                region_plot(args.hm, chrom, resolution, region=args.r, tads1=tads1, ratio=True)
            if tads is not None and tads1 is None:
                region_plot(args.hm, chrom, resolution, region=args.r, tads=tads, ratio=True)
            if tads is not None and tads1 is not None:
                region_plot(args.hm, chrom, resolution, region=args.r, tads=tads, tads1=tads1, ratio=True)
        else:
            if tads is None and tads1 is None:
                region_plot(args.hm, chrom, resolution, ratio=True)
            if tads is None and tads1 is not None:
                region_plot(args.hm, chrom, resolution, tads1=tads1, ratio=True)
            if tads is not None and tads1 is None:
                region_plot(args.hm, chrom, resolution, tads=tads, ratio=True)
            if tads is not None and tads1 is not None:
                region_plot(args.hm, chrom, resolution, tads=tads, tads1=tads1, ratio=True)
